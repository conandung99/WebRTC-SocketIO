...
(function() {
  ...
  },
  {
      url: 'turn:numb.viagenie.ca',
      credential: 'Dung1995',
      username: 'conandung9999@gmail.com'
  },
  {
  ...
  rtc.connect = function(server, room) {
    ...
    const wbs = server.replace(/^http/, 'wss');
    console.log('Create websocket: ' + wbs);
    rtc._socket = new WebSocket(wbs + ':9449');
  ...
  rtc.sendOffer = function(socketId) {
    ...
    pc.createOffer(sdpConstraints).then(function(session_description) {
  ...
  rtc.sendAnswer = function(socketId, sdp) {
    ...
    pc.createAnswer(sdpConstraints).then(function(session_description) {
      pc.setLocalDescription(session_description);
      rtc._socket.send(JSON.stringify({
